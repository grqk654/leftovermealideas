...
function HomePage() {
  ...
  return (
    <div>
      {/* Hero */}
      <div style={{ background: C.bgHero, padding: '56px 24px 48px', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ maxWidth: 640, position: 'relative', zIndex: 1, margin: '0 auto 0 0', paddingLeft: 24 }}>
          ...
          <p style={{ fontSize: 16, color: '#4a6a2a', lineHeight: 1.65, marginBottom: 28, maxWidth: 480, fontFamily: F.body }}>
            Search by whatever's in your fridge. Instant ideas — no food wasted, no sign-up needed.
          </p>
          <form onSubmit={handleSearch} style={{ display: 'flex', maxWidth: 540, background: '#fff', border: `1px solid ${C.greenBorder}`, borderRadius: 12, overflow: 'hidden', boxShadow: C.shadowMd }}>
            <input
  ...
  return (
    <div style={{ maxWidth: 800, margin: '0 auto', padding: '32px 24px 64px' }}>
      ...
      <div style={{ background: C.bgSoft, border: `1px solid ${C.greenBorder}`, borderRadius: 14, padding: 20, marginBottom: 40 }}>
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          {['Non-stick wok', 'Chef\'s knife', 'Meal prep containers'].map(t => (
            <a key={t} href="#" style={{ border: `1px solid ${C.greenBorder}`, borderRadius: 8, padding: '6px 12px', fontSize: 12, color: C.green, fontWeight: 500, fontFamily: F.body, textDecoration: 'none' }}>{t} →</a>
          ))}
        </div>
      </div>
