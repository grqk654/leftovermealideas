...
function GuidesPage() {
  ...
  const riceGuides = guides.filter(g => g.ingredientId === 'leftover-rice')
  const chickenGuides = guides.filter(g => g.ingredientId === 'leftover-chicken')
  ...
  return (
    <div style={{ maxWidth: 900, margin: '0 auto', padding: '40px 24px' }}>
      ...
      <p style={{ color: C.textMuted, fontSize: 15, fontFamily: F.body, marginBottom: 36, lineHeight: 1.6 }}>Recipe ideas, food safety tips, and everything you need to make the most of your leftovers.</p>

      {[['🍚', 'Leftover Rice Guides', riceGuides], ['🍗', 'Leftover Chicken Guides', chickenGuides]].map(([emoji, heading, items]) => (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            {items.map(g => (
              <Link key={g.id} to={`/guides/${g.slug}`} style={{ textDecoration: 'none' }}>
                <div style={{ border: `1px solid ${C.border}`, borderRadius: 14, padding: '18px 20px', background: C.bg, transition: 'all 0.15s' }}
                  onMouseEnter={e => { e.currentTarget.style.borderColor = C.greenMid; e.currentTarget.style.background = C.bgSoft }}
                  onMouseLeave={e => { e.currentTarget.style.borderColor = C.border; e.currentTarget.style.background = C.bg }}>
                  <div style={{ fontFamily: F.display, fontSize: 15, fontWeight: 600, color: C.text, marginBottom: 6, lineHeight: 1.3 }}>{g.title}</div>
                  <div style={{ fontSize: 13, color: C.textMuted, fontFamily: F.body, lineHeight: 1.5 }}>{g.excerpt}</div>
                  <div style={{ fontSize: 12, color: C.green, fontWeight: 500, fontFamily: F.body, marginTop: 10 }}>Read guide &#8594;</div>
                </div>
              </Link>
            ))}
          </div>
...
function GuidePage() {
  ...
  const ingredient = getIngredientBySlug(guide.ingredientId)
    ...
    <div style={{ maxWidth: 800, margin: '0 auto', padding: '32px 24px 64px' }}>
      ...
      <div style={{ margin: '20px 0 32px' }}>
        <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
          <Tag label={isChicken ? 'Leftover Chicken' : 'Leftover Rice'} color={isChicken ? '#FAEEDA' : C.greenLight} textColor={isChicken ? '#7a4a00' : C.greenDark} />
          <Tag label={guide.guideType} color="#f0f0f0" textColor="#555" />
        </div>
        ...
        <p style={{ fontSize: 16, color: C.textMuted, lineHeight: 1.7, fontFamily: F.body }}>{guide.intro}</p>
      </div>
      ...
      {guide.sections.map((section, i) => {
        ...
        return (
          <div key={i} style={{ marginBottom: 40 }}>
            <h2 style={{ fontFamily: F.display, fontSize: 22, fontWeight: 600, color: C.text, marginBottom: 12, letterSpacing: '-0.02em' }}>{section.heading}</h2>
            <p style={{ fontSize: 15, color: C.text, lineHeight: 1.75, fontFamily: F.body, marginBottom: sectionRecipes.length > 0 ? 20 : 0 }}>{section.body}</p>
            {sectionRecipes.length > 0 && (
              <div style={{ display: 'grid', gridTemplateColumns: `repeat(${Math.min(sectionRecipes.length, 3)},1fr)`, gap: 12 }}>
      ...
      {/* CTA back to ingredient */}
      {ingredient && (
        <div style={{ background: isChicken ? '#fffcf5' : C.bgSoft, border: `1px solid ${isChicken ? C.amberBorder : C.greenBorder}`, borderRadius: 14, padding: 24, marginTop: 16 }}>
