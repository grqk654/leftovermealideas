...
function GuidesPage() {
  const guides = guidesData.filter(g => g.published)
  const grouped = {
    storage: { label: 'Storage Tips', emoji: '🫙', guides: guides.filter(g => g.guideType === 'storage') },
    reheating: { label: 'Reheating Guides', emoji: '🔥', guides: guides.filter(g => g.guideType === 'reheating') },
    safety: { label: 'Food Safety', emoji: '✅', guides: guides.filter(g => g.guideType === 'safety') },
    ideas: { label: 'Recipe Ideas', emoji: '💡', guides: guides.filter(g => g.guideType === 'ideas') },
  }
  ...
  return (
    <div style={{ maxWidth: 1000, margin: '0 auto', padding: '40px 24px' }}>
      ...
      <p style={{ color: C.textMuted, fontSize: 15, fontFamily: F.body, marginBottom: 36, lineHeight: 1.6 }}>Storage tips, reheating guides, food safety, and recipe ideas — everything you need to make the most of your leftovers.</p>

      {Object.entries(grouped).map(([type, { label, emoji, guides: items }]) => {
        if (items.length === 0) return null
        return (
          <div key={type} style={{ marginBottom: 44 }}>
            <h2 style={{ fontFamily: F.display, fontSize: 20, fontWeight: 600, color: C.text, marginBottom: 16, display: 'flex', alignItems: 'center', gap: 8 }}>
              <span style={{ fontSize: 20 }}>{emoji}</span> {label}
            </h2>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 12 }}>
              {items.map(g => (
                <Link key={g.id} to={`/guides/${g.slug}`} style={{ textDecoration: 'none' }}>
                  <div style={{ border: `1px solid ${C.border}`, borderRadius: 14, padding: '16px 18px', background: C.bg, height: '100%', transition: 'all 0.15s', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}
                    onMouseEnter={e => { e.currentTarget.style.borderColor = C.greenMid; e.currentTarget.style.background = C.bgSoft }}
                    onMouseLeave={e => { e.currentTarget.style.borderColor = C.border; e.currentTarget.style.background = C.bg }}>
                    <div>
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
                        <Tag label={type === 'ideas' ? (g.ingredientId === 'leftover-chicken' ? 'Chicken' : 'Rice') : type} small />
                        {g.readTime && <span style={{ fontSize: 11, color: C.textLight, fontFamily: F.body }}>{g.readTime} min read</span>}
                      </div>
                      <div style={{ fontFamily: F.display, fontSize: 14, fontWeight: 600, color: C.text, marginBottom: 6, lineHeight: 1.3 }}>{g.title}</div>
                      <div style={{ fontSize: 13, color: C.textMuted, fontFamily: F.body, lineHeight: 1.5 }}>{g.excerpt}</div>
                    </div>
                    <div style={{ fontSize: 12, color: C.green, fontWeight: 500, fontFamily: F.body, marginTop: 12 }}>Read guide &#8594;</div>
                  </div>
                </Link>
              ))}
            </div>
...
function GuidePage() {
  const { slug } = useParams()
  const guide = guidesData.find(g => g.slug === slug && g.published)

  if (!guide) return <NotFound />

  const ingredient = guide.ingredientId ? getIngredientBySlug(guide.ingredientId) : null
  const isChicken = guide.ingredientId === 'leftover-chicken'

  const getSectionRecipes = (ids) => {
    if (!ids || ids.length === 0) return []
    return ids.map(id => getRecipeBySlug(id)).filter(Boolean)
  }
    ...
    <div style={{ maxWidth: 800, margin: '0 auto', padding: '32px 24px 64px' }}>
      ...
      <div style={{ margin: '20px 0 32px' }}>
        <div style={{ display: 'flex', gap: 8, marginBottom: 12, flexWrap: 'wrap' }}>
          <Tag label={guide.guideType} color={isChicken ? '#FAEEDA' : C.greenLight} textColor={isChicken ? '#7a4a00' : C.greenDark} />
          {guide.readTime && <span style={{ fontSize: 12, color: C.textMuted, fontFamily: F.body, display: 'flex', alignItems: 'center' }}>{guide.readTime} min read</span>}
        </div>
        <h1 style={{ fontFamily: F.display, fontSize: 32, fontWeight: 600, color: C.text, letterSpacing: '-0.03em', lineHeight: 1.2, marginBottom: 14 }}>{guide.title}</h1>
        <p style={{ fontSize: 16, color: C.textMuted, lineHeight: 1.7, fontFamily: F.body }}>{guide.description}</p>
      </div>

      {guide.sections.map((section, i) => {
        const sectionRecipes = getSectionRecipes(section.relatedRecipes)
        return (
          <div key={i} style={{ marginBottom: 36 }}>
            <h2 style={{ fontFamily: F.display, fontSize: 21, fontWeight: 600, color: C.text, marginBottom: 10, letterSpacing: '-0.02em' }}>{section.heading}</h2>
            <p style={{ fontSize: 15, color: C.text, lineHeight: 1.75, fontFamily: F.body, marginBottom: sectionRecipes.length > 0 ? 16 : 0 }}>{section.body}</p>
            {sectionRecipes.length > 0 && (
              <div style={{ display: 'grid', gridTemplateColumns: `repeat(${Math.min(sectionRecipes.length, 3)}, 1fr)`, gap: 12, marginTop: 16 }}>
                {sectionRecipes.map(r => <RecipeCard key={r.id} recipe={r} />)}
              </div>
            )}
          </div>
        )
      ...
      {guide.keyTakeaways && guide.keyTakeaways.length > 0 && (
        <div style={{ background: isChicken ? '#fffcf5' : C.bgSoft, border: `1px solid ${isChicken ? C.amberBorder : C.greenBorder}`, borderRadius: 14, padding: 24, marginBottom: 32 }}>
          <div style={{ fontFamily: F.display, fontSize: 16, fontWeight: 600, color: C.text, marginBottom: 14 }}>Key takeaways</div>
          <ul style={{ listStyle: 'none', padding: 0 }}>
            {guide.keyTakeaways.map((t, i) => (
              <li key={i} style={{ display: 'flex', gap: 10, alignItems: 'flex-start', marginBottom: i < guide.keyTakeaways.length - 1 ? 10 : 0, fontSize: 14, color: C.text, fontFamily: F.body, lineHeight: 1.5 }}>
      ...
      {ingredient && (
        <div style={{ background: isChicken ? '#fffcf5' : C.bgSoft, border: `1px solid ${isChicken ? C.amberBorder : C.greenBorder}`, borderRadius: 14, padding: 24 }}>
          <div style={{ fontFamily: F.display, fontSize: 16, fontWeight: 600, color: C.text, marginBottom: 6 }}>
            Browse all {ingredient.name} recipes
          </div>
          <div style={{ fontSize: 14, color: C.textMuted, fontFamily: F.body, marginBottom: 14 }}>
            {ingredient.name} has {getRecipesByIngredient(ingredient.id).length} recipes across every meal type.
          </div>
          <Link to={`/ingredients/${ingredient.slug}`} style={{ background: C.green, color: '#fff', borderRadius: 8, padding: '9px 18px', fontSize: 13, fontWeight: 500, fontFamily: F.body, textDecoration: 'none', display: 'inline-block' }}>
            See all recipes &#8594;
          </Link>
        </div>
      )}
